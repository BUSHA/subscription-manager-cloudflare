...
type ValidatedSubscription = Required<Pick<SubscriptionInput, "name" | "amount" | "currency" | "billing_cycle">> &
  Pick<
    SubscriptionInput,
    "description" | "billing_date" | "category" | "payment_method" | "url" | "notes" | "icon" | "color" | "tags"
  > & {
    is_active: boolean;
  };
...
async function createSubscription(request: Request, env: Env, userId: string) {
  ...
  const result = await env.DB.prepare(
    `INSERT INTO subscriptions (
      user_id, name, description, amount, currency, billing_cycle, billing_date,
      category, payment_method, url, notes, icon, color, tags, is_active
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  )
    .bind(
      userId,
...
async function updateSubscription(request: Request, env: Env, userId: string, id: number) {
  ...
  await env.DB.prepare(
    `UPDATE subscriptions SET
      name = ?, description = ?, amount = ?, currency = ?, billing_cycle = ?, billing_date = ?,
      category = ?, payment_method = ?, url = ?, notes = ?, icon = ?, color = ?, tags = ?,
...
async function parseAndValidate(request: Request): Promise<{ subscription: ValidatedSubscription } | { response: Response }> {
  ...
  const billingCycle = typeof input.billing_cycle === "string" ? input.billing_cycle.trim() : "";
  ...
  return {
    subscription: {
      ...
      billing_date: cleanString(input.billing_date),
      category: cleanString(input.category),
      payment_method: cleanString(input.payment_method),
